...
- **やること**:
  - [x] `claude_desktop.rs`: `%APPDATA%\Claude\config.json` (Win) / `~/Library/Application Support/Claude/config.json` (mac) の `oauth:tokenCacheV2`(無ければ `oauth:tokenCache`) を os_crypt 復号して accessToken を取得
    ...
    - mac: Keychain `"Claude Safe Storage"` のパスフレーズ → PBKDF2-HMAC-SHA1(saltysalt,1003,16) → AES-128-CBC (IV=0x20×16, PKCS7) — **コードレビュー済み / コンパイル検証は macOS CI 待ち (Windows からクロスコンパイル不可)**
  ...
  - **残**: ① macOS 実機/CI でのコンパイル ② macOS での end-to-end 確認 ③ UA バージョン文字列 (`claude-code/2.1.37`) の定期更新
- **メモ / リスク**:
