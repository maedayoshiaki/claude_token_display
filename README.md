...
Claude Max プランと Codex (ChatGPT) プランの使用枠を、macOS のメニューバー / Windows のシステムトレイに常時表示する常駐アプリ。**両プロバイダの 5h セッション / 週間制限を同時に 1 つのポップオーバーで確認できます**。
...
- 「TRAY」バッジが付いている側がトレイに % を表示しているプロバイダ
- リセット時刻の表記:
  - `in 30m` / `in 2h` / `in 2h30m` (24h 以内)
  - `Mon 09:00` (1 日以上先)
  - `now` (リセット中)

データはデフォルト 5 分おきに自動更新。手動更新はパネル右上の `⟳` ボタン、またはトレイ右クリック → "Refresh now"。

### カスタマイズ (⚙ 設定パネル)

- **トレイ表示**: メニューバー / システムトレイに % を出すプロバイダ (Claude / Codex)
- **更新間隔**: 1〜60 分
- **文字サイズ**: 0.6〜2.0 倍
- **Sonnet 週間を表示**: ON/OFF (OFF で Claude の Sonnet 部分を隠す)

設定は localStorage と Rust 側 atomic state に永続化されます。
